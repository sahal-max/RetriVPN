...
const resellerSvr = currentServers.filter(s => Number(s.is_reseller_only) === 1);
const memberSvr = currentServers.filter(s => Number(s.is_reseller_only) !== 1);
const fmtSvr = (server) => {
...
let serverList = '';
if (isR && resellerSvr.length > 0) {
  serverList += '🔐 *SERVER RESELLER*\n━━━━━━━━━━━━━━━━━━━━━━\n' + resellerSvr.map(fmtSvr).join('\n\n') + '\n\n';
}
if (memberSvr.length > 0) {
  if (isR && resellerSvr.length > 0) serverList += '👥 *SERVER MEMBER*\n━━━━━━━━━━━━━━━━━━━━━━\n';
  serverList += memberSvr.map(fmtSvr).join('\n\n');
}
serverList = serverList || '─ Tidak ada server tersedia ─';
    if (ctx.updateType === 'callback_query') {
...
const isErrorMsg = msg.includes('❌') || msgLower.includes('error') || msgLower.includes('tidak valid') || msgLower.includes('tidak ditemukan') || msgLower.includes('respons error') || msgLower.includes('sudah ada') || msgLower.includes('exists') || msgLower.includes('already exists');
if (isErrorMsg) {
